...
## Overview
This application, **Ajay Prefix Pro v1.0**, is a self-extracting archive (SFX) built using NSIS. It is designed to redirect user registry paths (e.g., Documents folder) on Windows systems, utilizing a 64-bit command-line environment for proper configuration. The tool includes custom scripts to manage this process efficiently.
...
## Usage Instructions
- **Extraction**: The SFX automatically extracts to `C:\` and runs `Setup.bat`. For manual control, extract to a preferred directory (e.g., `C:\AJAY_PREFIX_PRO\Ajay_Scripts`) and run `Setup_main.bat` with administrator privileges.
