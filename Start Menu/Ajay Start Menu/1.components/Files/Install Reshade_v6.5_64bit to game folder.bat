...
set "installname=Reshade_v6.5_64bit"
color 0a
echo *** wait....***
...
echo msgbox "Hello! Extract it in game exe folder , It's for dx9, dx10, dx11, dx12 and Opengl 64bit games !" , vbinformation+vbSystemModal > %tmp%\tmp.vbs
start /b cscript //nologo %tmp%\tmp.vbs
timeout.exe /t 1 >nul
del %tmp%\tmp.vbs
call "C:\Windows\Ajay_drive.bat" >nul 2>&1
Start /wait %drive_letter%:\Ajay_prefix\wget_files\temp\Reshade_v6.5_64bit.exe
timeout.exe /t 3 >nul
echo ************************************************
