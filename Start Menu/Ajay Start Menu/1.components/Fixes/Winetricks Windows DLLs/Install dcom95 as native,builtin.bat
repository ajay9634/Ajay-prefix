...
echo [INFO] Setting Wine version to win95...
reg add "HKEY_CURRENT_USER\Software\Wine" /v "Version" /t REG_SZ /d "win95" /f >nul 2>&1

timeout.exe /t 6 /nobreak >nul 2>&1

reg add "HKEY_CURRENT_USER\Software\Wine" /v "Version" /t REG_SZ /d "win95" /f >nul 2>&1
timeout.exe /t 4 /nobreak >nul 2>&1
echo [INFO] Wine version has been set to win95 successfully.
Start /wait "" "%drive_letter%:\Ajay_prefix\wget_files\temp\dcom95.exe"
