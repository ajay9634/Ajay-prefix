...
IF ERRORLEVEL 1 (
    ...
    C:\Windows\7z.exe x %drive_letter%:\Ajay_prefix\wget_files\d3d\dxvk-async-%version%.tar.gz -oC:\windows\temp -y
C:\Windows\7z.exe x C:\windows\temp\dxvk-async-%version%.tar -oC:\windows\temp -y
    IF ERRORLEVEL 1 (
        echo.
