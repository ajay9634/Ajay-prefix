...
wget -q -O D:/Ajay_prefix/wget_files/temp/all_async_releases.json https://api.github.com/repos/Sporif/dxvk-async/releases
...
IF NOT EXIST "D:\Ajay_prefix\wget_files\d3d\dxvk-async-%version%.tar.gz" (
    wget -q --show-progress -P D:/Ajay_prefix/wget_files/temp/ --progress=dot:mega https://github.com/Sporif/dxvk-async/releases/download/%version%/dxvk-async-%version%.tar.gz )
    
    IF EXIST "D:\Ajay_prefix\wget_files\temp\dxvk-async-%version%.tar.gz" (
    copy /y D:\Ajay_prefix\wget_files\temp\dxvk-async-%version%.tar.gz D:\Ajay_prefix\wget_files\d3d\dxvk-async-%version%.tar.gz )


color 1f
echo *** Extracting ...***
D:\Ajay_prefix\.Resources\winrar.exe x D:\Ajay_prefix\wget_files\d3d\dxvk-async-%version%.tar.gz C:\windows\temp\ -r -y >NUL 2>&1
echo *** installing...***
...
setlocal EnableDelayedExpansion
set count=0
for /f "tokens=2 delims=:" %%A in ('findstr /i "tag_name" D:/Ajay_prefix/wget_files/temp/all_async_releases.json 2^>nul') do (
    set "ver=%%A"
    set "ver=!ver:,=!"
    set "ver=!ver:"=!"
    set "ver=!ver: =!"
